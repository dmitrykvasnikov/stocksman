...
The Tauri 2 desktop shell starts and supervises a Rust/Tokio backend companion process over an OS-assigned loopback port. The backend applies versioned SQLite migrations and persists typed user preferences. Provider-neutral market-data contracts define instruments, intervals, candle history, live upserts, and validated canonical candles. An offline mock provider supplies deterministic history and replay streams for development without network access. The Binance Spot adapter fetches public historical klines over HTTPS, normalizes live WebSocket updates at the provider boundary, and recovers gaps and revisions after reconnects. The desktop workbench loads the five configured Binance pairs and all 16 Spot kline intervals into a configurable candlestick and volume chart with price and time axes and OHLCV hover details. Continuous integration verifies Linux and Windows desktop builds. The next pending work is tracked in [`PLAN.md`](PLAN.md).
...
Optional `start_timestamp` and `end_timestamp` values are forwarded as inclusive millisecond boundaries. The adapter enforces Binance's 1,000-row maximum, preserves UTC opening timestamps, parses the exchange's decimal strings into finite canonical OHLCV values, and determines the `closed` flag from each kline's closing time. The desktop chart loads its configured BTCUSDT, ETHUSDT, BNBUSDT, SOLUSDT, and XRPUSDT catalog and the complete 1-second-through-1-month interval mapping from this adapter, then requests the newest 80 candles for the selected series.
...
Subscriptions reconnect with bounded exponential backoff. Every reconnect performs an inclusive REST overlap from the newest observed candle, and a jump in live timestamps triggers recovery through the incoming candle. Recovery is paginated, honors `Retry-After` when Binance rate limits it, suppresses exact overlap duplicates, and still emits same-timestamp changes as revisions. Dropping the subscription interrupts both reconnect and rate-limit waits.
